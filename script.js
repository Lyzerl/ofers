document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMenuItems() {
        ...
        categories.forEach(category => {
            if (category.count > 0) { 
                const categoryItems = priceListData.filter(item => item[0].trim() === category.name.trim() && parseFloat(item[2]) <= category.maxPrice);
                ...
                // הוספת כפתור ה-"הוסף" לכל קטגוריה
    ...
    function addExtraItem(category, categoryItemsDiv) {
        ...
        select.appendChild(defaultOption);
        const allCategoryItems = priceListData.filter(item => item[0].trim() === category.trim());
